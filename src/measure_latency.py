...
def main():
    ...
    ap.add_argument("--max_length", type=int, default=128) # Matched train/predict length
    ap.add_argument("--runs", type=int, default=50)
    ap.add_argument("--device", default="cpu") # Force CPU for latency test
    args = ap.parse_args()

    tokenizer = AutoTokenizer.from_pretrained(args.model_dir if args.model_name is None else args.model_name)
    model = AutoModelForTokenClassification.from_pretrained(args.model_dir)

    # --- CRITICAL UPDATE: QUANTIZATION ---
    if args.device == "cpu":
        # print("Quantizing model for latency check...")
    ...
    print(f"  p95: {p95:.2f} ms") # Target: <= 20.00 ms
